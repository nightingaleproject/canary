class SystemsController < ApplicationController
  before_action :new_system, only: [:new]
  ...
  # Show specific EDRS results.
  def show
  end

  # Create new EDRS results.
  def new
  ...
  def ije_export_snapshot
    ije_export_data = {}
    @system.ije_export_results.limit(7).each do |r|
  ...
  def fhir_export_snapshot
    fhir_export_data = {}
    @system.fhir_export_results.limit(7).each do |r|
  ...
  def cda_export_snapshot
    cda_export_data = {}
    @system.cda_export_results.limit(7).each do |r|
  ...
  def ije_import_snapshot
    ije_import_data = {}
    @system.ije_import_results.limit(7).each do |r|
  ...
  def fhir_import_snapshot
    fhir_import_data = {}
    @system.fhir_import_results.limit(7).each do |r|
  ...
  def cda_import_snapshot
    cda_import_data = {}
    @system.cda_import_results.limit(7).each do |r|
  ...
  def new_system
    name = params[:name].present? ? params[:name] : 'Untitled EDRS'
    @system = System.create(creator: current_user, name: name)
  end
